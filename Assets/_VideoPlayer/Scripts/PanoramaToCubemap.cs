...
public class PanoramaToCubemap : EditorWindow {
	 ...
	 */
	public void ConvertPanoramaToCubemap(Texture2D imageToCovert) {
		m_srcTexture = imageToCovert;
		// 一時的にテクスチャのGetPixelを有効にする.
		string assetPath = Application.dataPath +"/_VideoPlayer/Sprites";
		...
		string filePath = PanoramaToCubemap.outputImageDirectory + "/" + "360_auto_detection_test_"+Time.time.ToString("f6") ;
...
/*
    public void Split(Texture2D image, int width, int height, string filename)
    {
